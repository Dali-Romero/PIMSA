...
router.get('/', isLoggedIn, IsAuthorized('monitor'), async (req, res) => {
    ...
    const ordenes = await pool.query('SELECT Ordenes.ordenId, Ordenes.fechaGen, Ordenes.fechaEnt, GROUP_CONCAT(DISTINCT CONCAT(TareasOrden.tareaOrdenId, "-", ProductosJuntos.productoNombre) ORDER BY TareasOrden.tareaOrdenId) AS producto, COUNT(IF(Tareas.terminada = 1, 1, NULL)) AS tareasTerminadas, COUNT(Tareas.tareaId) AS totalTareas, Cotizaciones.empleado FROM Tareas INNER JOIN TareasOrden ON Tareas.tareaorden_id = TareasOrden.tareaOrdenId INNER JOIN Ordenes ON TareasOrden.orden_id = Ordenes.ordenId INNER JOIN Cotizaciones ON Ordenes.cot_id = Cotizaciones.cotId INNER JOIN ((SELECT Ordenes.ordenId AS orden_id, ProductosCotizados.prodCotizadoId AS prodCotizadoId, Productos.nombre AS productoNombre, 0 AS fueraCatalogo FROM Ordenes INNER JOIN Cotizaciones ON Ordenes.cot_id = Cotizaciones.cotId INNER JOIN ProductosCotizados ON Cotizaciones.cotId = ProductosCotizados.cot_id INNER JOIN Productos ON ProductosCotizados.producto_id = Productos.productoId) UNION ALL (SELECT Ordenes.ordenId AS orden_id, FueraCatalogoCotizados.fueraCotizadoId AS prodCotizadoId, FueraCatalogoCotizados.concepto AS productoNombre, 1 AS fueraCatalogo FROM Ordenes INNER JOIN Cotizaciones ON Ordenes.cot_id = Cotizaciones.cotId INNER JOIN FueraCatalogoCotizados ON Cotizaciones.cotId = FueraCatalogoCotizados.cot_id)) AS ProductosJuntos ON Ordenes.ordenId = ProductosJuntos.orden_id AND TareasOrden.fueracatalogo = ProductosJuntos.fueraCatalogo AND TareasOrden.cotizadoId = ProductosJuntos.prodCotizadoId GROUP BY Ordenes.ordenId;');
    
    // ajustar estructura de los productos CHECK
