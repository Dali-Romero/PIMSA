...
router.get('/infoclient/:id', isLoggedIn, IsAuthorized('seeListClients'), async(req, res)=>{
    ...
    const suma = await pool.query('SELECT SUM(Cotizaciones.total) as suma  FROM Cotizaciones INNER JOIN Ordenes ON Cotizaciones.cotId = Ordenes.cot_id WHERE Cotizaciones.cliente_id = ?;', [id]);
