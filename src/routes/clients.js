...
router.get('/infoclient/:id', isLoggedIn, IsAuthorized('seeListClients'), async(req, res)=>{
    ...
    const deuda = await pool.query('SELECT SUM(Cotizaciones.total) as deuda FROM Cobranza INNER JOIN Ordenes ON Cobranza.orden_id = Ordenes.ordenId INNER JOIN Cotizaciones ON Ordenes.cot_id = Cotizaciones.cotId WHERE Cotizaciones.cliente_id = ? AND Cobranza.estatus IS NOT "Cobranza realizada";',[id]);
    const suma = await pool.query('SELECT SUM(Cotizaciones.total) as suma  FROM Cotizaciones INNER JOIN Ordenes ON Cotizaciones.cotId = Ordenes.cot_id WHERE Cotizacion.cliente_id = ?;', [id]);
