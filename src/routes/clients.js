...
router.get('/infoclient/:id', isLoggedIn, IsAuthorized('seeListClients'), async(req, res)=>{
    ...
    const cotizacion = await pool.query( 'SELECT Cotizaciones.*, Clientes.clienteId, Clientes.nombre FROM Cotizaciones INNER JOIN Clientes ON Cotizaciones.cliente_id = Clientes.clienteId WHERE Clientes.clienteId = ?;', [id] );
