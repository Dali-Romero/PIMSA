...
router.get('/infoclient/:id', isLoggedIn, IsAuthorized('seeListClients'), async(req, res)=>{
    ...
    const deuda = await pool.query('SELECT SUM(total) as deuda FROM Cobranza INNER JOIN Cotizaciones ON Ordenes.cot_id = Cotizaciones.cotId WHERE Cotizaciones.cliente_id = ? AND Cobranza.estatus <> "Cobranza realizada";',[id]);
