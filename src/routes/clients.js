...
router.get('/infoclient/:id', isLoggedIn, IsAuthorized('seeListClients'), async(req, res)=>{
    ...
    const deuda = await pool.query('SELECT SUM(total) as deuda FROM Cobranza INNER JOIN Ordenes ON Cobranza.orden_id = Ordenes.ordenId INNER JOIN Cotizaciones ON Ordenes.cot_id = Cotizaciones.cotId WHERE Cotizaciones.cliente_id = ? AND Cotizaciones.estatus <> "Cobranza realizada";',[id]);
    const suma = await pool.query('SELECT SUM(total) as suma  FROM Cotizaciones WHERE Cotizaciones.cliente_id = ? AND Cotizaciones.estatus <> "Cancelada";', [id]);
