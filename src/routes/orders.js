...
const { dimensiones, moneda, createPdfOrders } = require('../lib/helpers.js');
...
router.get('/', isLoggedIn, IsAuthorized('seeListOrders'), async (req, res)=>{
    const ordenes = await pool.query('SELECT Ordenes.ordenId, fechaGen, IF(DATE(Ordenes.fechaEnt) = DATE("2001-01-01"), "-", Ordenes.fechaEnt) AS fechaEnt, Empleados.nombreComp, Ordenes.terminada, Ordenes.estatus, Cobranza.folio FROM ((Ordenes INNER JOIN Usuarios ON Ordenes.usuario_id = Usuarios.usuarioId) INNER JOIN Empleados ON Usuarios.empleado_id = Empleados.empleadoId LEFT JOIN Cobranza ON Ordenes.ordenId = Cobranza.orden_id);');
...
router.get('/download/:id', isLoggedIn, IsAuthorized('seeListOrders'), async (req, res)=>{
    ...
    const stream = res.writeHead(200, {
        ...
        'content-Disposition': `attachment; filename=ot${id}_${timeMilis}.pdf`
    });
    
    createPdfOrders(dataDb, (data) => {
        stream.write(data)
    }, () => {
        stream.end();
    });
